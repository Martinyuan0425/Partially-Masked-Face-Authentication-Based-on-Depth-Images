...
def facenet(input_shape, num_classes = None, backbone = "inception_resnetv2", mode = "train"):
    ...
    if backbone=="mobilenet":
        ...
        model = InceptionResNetV2(inputs)
    else:
...
def face_analysis_model(input_shape, num_classes = None, backbone = "mobilenet", mode = "train"):
    inputs = Input(shape=input_shape)
    #--------------------------------------------#
    #   利用主干网络进行特征提取
    #--------------------------------------------#
    ...
    if mode == "train":
        ...
        #--------------------------------------------#
        logits          = Dense(num_classes)(model.output)
        combine_model   = Model(inputs, logits)
        return combine_model
    elif mode == "predict":
        #--------------------------------------------#
        #   预测的时候只需要考虑人脸的特征向量就行了
